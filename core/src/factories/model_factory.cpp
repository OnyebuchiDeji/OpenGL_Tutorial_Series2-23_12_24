...
/**
 * @brief
 * This is different because it loads many 2D images as opposed to the previous
 * version that loaded just one 
 * 
 * @return unsigned int 
 */
unsigned int MeshFactory::load_materials() {

    unsigned int material;

    glGenTextures(1, &material);
    glBindTexture(GL_TEXTURE_2D_ARRAY, material);
    glTexStorage3D(GL_TEXTURE_2D_ARRAY, 1, GL_RGBA8, 1024, 1024, objectTypeCount);
    ...
    for (uint32_t i=0; i < objectTypeCount; ++i)
    {
        int width, height, channels;
        stbi_set_flip_vertically_on_load(true);
        unsigned char* data = stbi_load(imageNames[i], &width, &height, &channels, STBI_rgb_alpha);

        glTexSubImage3D(GL_TEXTURE_2D_ARRAY,
            0, 0, 0, i, 1024, 1024, 1, GL_RGBA, GL_UNSIGNED_BYTE, data);

        //  free data from stbi after loading it to OPenGL
        stbi_image_free(data);
    }
