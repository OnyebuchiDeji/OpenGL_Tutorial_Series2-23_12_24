...
{
    for (auto& [object, VBO] : m_VBOs)
    {
        glDeleteBuffers(1, &VBO);
    }
    for (auto& [object, VAO] : m_VAOs)
    {
        glDeleteVertexArrays(1, &VAO);
    }
    for (auto& [object, Texture] : m_Textures)
    {
...
)
{
    ...
    if (m_VAOs.contains(objectType))
    {
        render.VAO = m_VAOs[objectType];
        render.vertexCount = m_VertexCounts[objectType];
        return render;
    }
    ...
    m_VertexCounts[objectType] = 36;

    unsigned int VAO;
    glGenVertexArrays(1, &VAO);
    m_VAOs[objectType] = VAO;
    glBindVertexArray(VAO);

    unsigned int VBO;
    glGenBuffers(1, &VBO);
    m_VBOs[objectType] = VBO;
    ...
    glm::mat4 preTransform)
{
    ...
    m_VertexCounts[objectType] = m_Vertices.size() / 8;

    render.VAO = VAO;
    render.vertexCount = m_Vertices.size() / 8;
    m_V.clear();
    m_Vt.clear();
    m_Vn.clear();
    m_Vertices.clear();
...
{ 
    ...
    if (m_Textures.contains(objectType)) {
