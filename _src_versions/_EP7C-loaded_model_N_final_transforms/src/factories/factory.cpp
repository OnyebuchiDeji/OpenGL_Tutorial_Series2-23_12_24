...
void Factory::make_girl_model(glm::vec3 position, glm::vec3 eulers)
{
    ...
    */
    ...
    RenderComponent render = make_obj_mesh("../_resources/models/girl.obj", preTransform);
...
RenderComponent Factory::make_obj_mesh(const char* filename, glm::mat4 preTransform)
{
    ...
    while (std::getline(file, line))
    {
        ...
        if (!words[0].compare("v")){
            //  this adds w = 1.0f to apply a translation if desired
            ...
            //  This adds w = 0.0f because no translation component is needed to apply rotation
...
{
    //  The below starts from index 1, because index 0 is the label...
    //  e.g. v, vt, or vn.
    //  std::stof() converts a string to a floating point number
