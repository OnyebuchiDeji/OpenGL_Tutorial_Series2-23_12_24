...
StaticMesh MeshFactory::build_obj_mesh()
{
    ...
    //  it's used in the VertexAttribPointer function.
...
void MeshFactory::reserve_space(const char* filepath)
{
    ...
    while (std::getline(file, line))
    {
        ...
        if (!words[0].compare("v")){
            ...
            //  The number of vertex data that make a point: 8, like, in this case: x, y, z, u, v, r, g, b
            //  Number of floats per triangle: 3
            dataCount += 3 * 8 * (words.size() - 3);
        }
    }
    file.close();
