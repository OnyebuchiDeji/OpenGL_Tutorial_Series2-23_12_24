...
RenderSystem::RenderSystem(std::vector<unsigned int>& shaders, GLFWwindow* window,
    ...
    m_Shaders(shaders), m_Transforms(transforms), m_Renderables(renderables), m_Animations(animations)
{
...
void RenderSystem::update() {
    ...
    glUseProgram(m_Shaders[1]);
    //  Bund the Static Geometry's textures
    ...
    //  Create a Window Called "GUI Tool, with a menu bar
    ImGui::Begin("GUI Tool", &show_window, ImGuiWindowFlags_MenuBar);
    ...
    //  Wdit a color stored as 4 floats
    ImGui::ColorEdit4("Color", m_MyColor);
    ...
    ImGui::EndChild();
    ImGui::End();
    ImGui::Render();    //  prepare the internal structures
    //  This then tells opengl to draw stuff
