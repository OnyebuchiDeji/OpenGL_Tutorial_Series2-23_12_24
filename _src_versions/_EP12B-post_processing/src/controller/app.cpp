...
void App::set_up_opengl()
{
    ...
    m_Shaders.push_back(make_shader_program(
        "../core/src/shaders/vertex_post.txt",
        "../core/src/shaders/fragment_post.txt"
    ));
    ...
    m_DepthBuffer = factory.build_depthbuffer(m_Window);
    m_FrameBuffer = factory.build_framebuffer(m_ColorBuffer, m_DepthBuffer);
}
...
void App::make_systems()
{
    ...
    m_RenderSystem = new RenderSystem(m_Shaders, m_Window, m_TransformComponents,
        m_RenderComponents, m_AnimationComponents, m_FrameBuffer);
    m_PostSystem = new PostSystem(m_Shaders[3], m_ColorBuffer);
}
